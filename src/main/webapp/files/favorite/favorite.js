...
.always(function() {
	...
	$("#u1167 p span").html();
	$("#u1171 p span").html();
	$("#u1173 p span").html('【登录】 |');
	$("#u1175 p span").html('【注册】');
}
...
//已经做过添加到后台收藏夹的操作
				...
				//获取收藏夹的总数，将其存储在session中以便全局获取该变量
					...
					"<td>"+val.price+"</td>"+
				 	"<td align='center' goodsId="+val.goodsId+">"+
				 		"<input type='button' id='moveToCart' value='加入购物车' onclick='moveToCartList(this)'>"+
				 		"<input type='button' id='remove' value='删除' onclick='deleteFavoriteItem(this)'>"+
				 	"</td>"+
				 	"</tr>";
	...
	//删除收藏夹中数据行
	function delelteFavoriteItem(btn){
